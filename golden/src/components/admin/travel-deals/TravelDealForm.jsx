...
export default function TravelDealForm() {
  const { country_slug, slug } = useParams();
  ...
  // Fetch existing deal if editing
  useEffect(() => {
    if (slug) {
      axios
  ...
  function handleChange(e) {
    ...
    if (type === "file") {
      ...
      setFormData((prev) => ({
        ...prev,
        themes: value.split(",").map((t) => t.trim()).filter(t => t !== ""),
      }));
    } else {
      setFormData((prev) => ({
        ...prev,
  ...
  async function handleSubmit(e) {
    ...
    // Append all fields except themes and country
    Object.entries(formData).forEach(([key, val]) => {
      if (key === "themes" || key === "image") return; // handle later
      if (val !== null && val !== "") {
        data.append(key, val);
      }
    });

    // Append country as its ID (integer)
    data.append("country", countryId);

    // Append themes as JSON string
    ...
    try {
      ...
      setSaving(false);
      setError("Failed to save travel deal. Please check the data and try again.");
