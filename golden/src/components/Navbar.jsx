    ...
    <header className="navbar">
      <div className="navbar-container">
        ...
        <nav className="navbar-links">
          <div className="dropdown" onMouseEnter={() => setShowDestinations(true)} onMouseLeave={() => setShowDestinations(false)}>
            ...
            {showDestinations && activeRegion && (
              <div className="mega-menu-dest no-search">
                <div className="mega-columns">
                  <div className="column">
                    <h4>Destinations</h4>
                    <ul>
                      {regions.map((region, idx) => (
                        <li key={idx} onClick={() => setActiveRegion(region)} className={activeRegion === region ? "active" : ""}>{region}</li>
                      ))}
                    </ul>
                  </div>
                  ...
                  <div className="column">
                    ...
                    <ul>
                      {(countriesByRegion[activeRegion] || []).map((country, idx) => (
                        <li key={idx}><Link to={`/destinations/${country.slug}`} className="plain-link">{country.name}</Link></li>
                      ))}
                    </ul>
                  </div>
                  ...
                  <div className="column image-column">
                    ...
                    <p className="image-description">
                      Discover unforgettable journeys in <strong>{activeRegion}</strong>. Whether you love beaches,
                      mountains, or cities, we’ve got something magical waiting for you.
                    </p>
          ...
          <div className="dropdown" onMouseEnter={() => setShowWaysToTravel(true)} onMouseLeave={() => setShowWaysToTravel(false)}>
            ...
            {showWaysToTravel && (
              <div className="mega-menu-ways no-search">
                <div className="mega-columns">
                  <div className="column">
                    <h4>Travel Types</h4>
                    <ul>
                      {travelTypes.map((type, idx) => (
                        <li key={idx} onClick={() => setActiveTravelType(type)} className={activeTravelType === type ? "active" : ""}>{type}</li>
                      ))}
                    </ul>
                  </div>
                  ...
                  <div className="column">
                    <h4>Top Options</h4>
                    <ul>
                      {(travelOptions[activeTravelType] || []).map((option, idx) => (<li key={idx}>{option}</li>))}
                    </ul>
                  </div>
                </div>
              </div>
          ...
          <div className="dropdown" onMouseEnter={() => setShowDeals(true)} onMouseLeave={() => setShowDeals(false)}>
            ...
            {showDeals && (
              <div className="mega-menu-deals no-search">
                <div className="mega-columns">
                  <div className="column">
                    <h4>Deals</h4>
                    <ul>
                      {dealCategories.map((cat, idx) => (
                        <li key={idx} onClick={() => setActiveDealCategory(cat)} className={cat === activeDealCategory ? "active" : ""}>{cat}</li>
                      ))}
                    </ul>
                  </div>
                  ...
                  <div className="column">
                    <h4>Top Offers</h4>
                    <ul>
                      {(dealItems[activeDealCategory] || []).map((deal, idx) => (<li key={idx}>{deal}</li>))}
                    </ul>
                  </div>
                </div>
              </div>
        ...
        <div className="navbar-icons">
          {showSearchIcon && (
            <button aria-label="Toggle Search" className="search-icon" onClick={() => setShowSearchBar((prev) => !prev)}>
              <Search size={20} />
            </button>
          )}
